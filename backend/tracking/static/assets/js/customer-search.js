...
(function($) {
    ...
    function bindEvents() {
        ...
        // Filters
        $('#filterCustomerType').on('change', function() {
        ...
        $('#clearSearchBtn').on('click', function() {
            ...
            $('#sortBy').val('recent');
            currentFilters = { type: '', sort: 'recent' };
            searchResults = [];
            updateSummaries([]);
            clearSearchResults();
            setActiveChips();
        ...
        $('#newStatus').on('change', function() {
            ...
            if (status === 'completed') {
                ...
                $('#completionAlert').addClass('d-none');
            }
        });

        // Create new order button
        $('#createNewOrderBtn').on('click', function() {
            if (currentCustomer) createNewOrderForCustomer();
        });
    }
    ...
    window.quickFilter = function(type) {
        ...
        try {
            ...
            if (type === 'today') {
                const todaysOrders = TrackingSystem.getAllOrders().filter(order => new Date(order.arrivalTime).toDateString() === today);
                const todaysCustomerIds = [...new Set(todaysOrders.map(o => o.customerId))];
                results = todaysCustomerIds.map(id => ({ customer: TrackingSystem.getCustomerById(id), orders: TrackingSystem.getOrdersByCustomer(id) })).filter(r => r.customer);
            } else if (type === 'active') {
                const activeOrders = TrackingSystem.getAllOrders().filter(order => !['completed','cancelled'].includes(order.status));
                const activeCustomerIds = [...new Set(activeOrders.map(o => o.customerId))];
                results = activeCustomerIds.map(id => ({ customer: TrackingSystem.getCustomerById(id), orders: TrackingSystem.getOrdersByCustomer(id) })).filter(r => r.customer);
                ...
                results = pendingCustomerIds.map(id => ({ customer: TrackingSystem.getCustomerById(id), orders: TrackingSystem.getOrdersByCustomer(id) })).filter(r => r.customer);
            }
            searchResults = results;
            displaySearchResults();
            const filterLabels = { 'today': "Today's Customers", 'active': 'Active Orders', 'pending': 'Pending Departure' };
    ...
    function setActiveChips(active) {
        $('#chipToday,#chipActive,#chipPending').removeClass('active');
        if (active === 'today') $('#chipToday').addClass('active');
        if (active === 'active') $('#chipActive').addClass('active');
        if (active === 'pending') $('#chipPending').addClass('active');
    }
                ...
                <div class="col-xl-6 col-lg-12 mb-4">
                    <div class="customer-card card h-100">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6">
                                    <p class="mb-2"><i data-feather="phone" class="me-2"></i>${customer.phone}</p>
                                    ${customer.email ? `<p class="mb-2"><i data-feather="mail" class="me-2"></i>${customer.email}</p>` : ''}
                                    <p class="mb-2"><i data-feather="calendar" class="me-2"></i>Total Orders: ${orders.length}</p>
                                </div>
                                <div class="col-md-6">
                                    ${lastOrder ? `
                                        <p class="mb-2"><i data-feather="clock" class="me-2"></i>Last Visit: ${formatDate(lastOrder.arrivalTime)}</p>
                                        <p class="mb-2"><span class="order-status status-${lastOrder.status}">${getStatusLabel(lastOrder.status)}</span></p>
                                ...
                                <div class="mt-3">
                                    ...
                                    ${activeOrders.slice(0, 2).map(order => `
                                        <div class="d-flex justify-content-between align-items-center mb-2 p-2 bg-light rounded">
                                            <div>
                                                <small class="fw-bold">${order.orderNumber}</small><br>
                            ...
                            <div class="quick-actions">
                                <div class="d-flex gap-2 flex-wrap">
                                    <button class="btn btn-primary btn-sm" onclick="viewCustomerDetails('${customer.id}')"><i data-feather="eye" class="me-1"></i>View Details</button>
                                    <button class="btn btn-success btn-sm" onclick="createOrderForCustomer('${customer.id}')"><i data-feather="plus" class="me-1"></i>New Order</button>
                                    ${activeOrders.length > 0 ? `<button class="btn btn-warning btn-sm" onclick="manageOrders('${customer.id}')"><i data-feather="settings" class="me-1"></i>Manage Orders</button>` : ''}
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    function applyFiltersAndSorting() {
        ...
        if (currentFilters.type) {
            list = list.filter(r => (r.customer.customerType || '').toLowerCase() === currentFilters.type);
        }
        switch (currentFilters.sort) {
            ...
            default: {
                const last = (r) => {
                    if (!r.orders || r.orders.length === 0) return 0;
                    const dt = r.orders[r.orders.length-1].arrivalTime;
    ...
    function updateSummaries(list) {
        ...
        let active = 0; let completedToday = 0;
        const todayStr = new Date().toDateString();
        list.forEach(r => {
            const activeOrders = r.orders.filter(o => !['completed','cancelled'].includes(o.status));
            active += activeOrders.length > 0 ? 1 : 0;
            completedToday += r.orders.filter(o => o.status==='completed' && o.departureTime && new Date(o.departureTime).toDateString()===todayStr).length;
        });
        $('#summaryTotal').text(`Total: ${total}`);
        $('#summaryActive').text(`Active Orders: ${active}`);
        $('#summaryCompletedToday').text(`Completed Today: ${completedToday}`);
    }
    ...
    window.createOrderForCustomer = function(customerId) {
        window.location.href = `/orders/create/?customerId=${customerId}`;
    };

    function createNewOrderForCustomer() {
        if (currentCustomer) {
            $('#customerDetailsModal').modal('hide');
            window.location.href = `/orders/create/?customerId=${currentCustomer.id}`;
        }
    }
