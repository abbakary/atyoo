...
var TrackingDashboard = (function() {
    ...
    function init() {
        ...
        // Hide Quick Actions card if present (requested removal)
        try {
        ...
        // Expose manual refresh handler
        if (typeof window !== 'undefined') {
            window.refreshRecentActivity = loadRecentActivities;
        }
    }
    ...
    function updateStatistics() {
        ...
        const activeOrders = orders.filter(order => ['pending','in-progress','service-complete','ready-for-departure'].includes(order.status));
        const completedToday = orders.filter(order => order.status === 'completed' && order.departureTime && new Date(order.departureTime).toDateString() === today);
        const arrivalsToday = orders.filter(order => order.arrivalTime && new Date(order.arrivalTime).toDateString() === today).length;

        // Average wait time for active orders (in minutes)
        let avgWaitMin = 0;
        if (activeOrders.length > 0) {
            const now = new Date();
            const waits = activeOrders.map(o => Math.max(0, Math.floor((now - new Date(o.arrivalTime)) / 60000)));
            avgWaitMin = Math.floor(waits.reduce((a,b)=>a+b,0) / waits.length);
        }
        const formatMins = (m) => {
            if (m < 60) return `${m}m`;
        ...
        // Match current index.html ids
        updateCounter('#liveCustomersToday', arrivalsToday);
        updateCounter('#liveActiveOrders', realtimeData.activeOrders);
        updateCounter('#liveCompletedToday', realtimeData.completedToday);
        const avgEl = document.querySelector('#liveAvgWaitTime');
        if (avgEl) avgEl.textContent = formatMins(avgWaitMin);

        // Service breakdown counters
        const normType = (o) => {
            if (o.serviceType) return o.serviceType;
            const s = (o.service || '').toLowerCase();
            if (s.includes('tire')) return 'tire-sales';
            if (s.includes('car')) return 'car-service';
            if (s.includes('inquiry') || s.includes('consult')) return 'consultation';
            return 'general-inquiry';
        };
        const car = orders.filter(o => normType(o) === 'car-service').length;
        const tire = orders.filter(o => normType(o) === 'tire-sales').length;
        const consult = orders.filter(o => normType(o) === 'consultation').length;
        const ready = orders.filter(o => o.status === 'ready-for-departure').length;
        updateCounter('#carServiceCount', car);
        updateCounter('#tireSalesCount', tire);
        updateCounter('#consultationCount', consult);
        updateCounter('#readyForDeparture', ready);
    }
